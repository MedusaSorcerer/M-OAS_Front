...
layui.define(["layer"], function (exprots) {
    ...
    var okUtils = {
         ...
         */
        ajax: function (url, type, params, load, headers = true, contentType) {
            var deferred = $.Deferred();
            var loadIndex;
            var headersObj = {};
            if (headers) {
                headersObj['Authorization'] = okUtils.local('M&OAS-token');
            }
            if (contentType) {
            ...
            $.ajax({
                ...
                error: function (data) {
                    ...
                    if (data.status === 401) {
                        ...
                        if (window.top.location.pathname.indexOf('login.html') === -1) {
